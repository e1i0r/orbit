...
func failedRun(t *testing.T) (Session, string) {
	...
	addTask(t, s, r, "PAY-1",
		...
		record.Event{
			...
			Phase: "implement",
			Data:  map[string]string{"engine": "claude", "model": "opus"},
		},
		record.Event{
			At:    at(5),
			Kind:  record.PhaseThought,
			Phase: "implement",
			Text:  "the handler keys on the request id",
		},
		record.Event{
			At:    at(6),
		...
		record.Event{
			...
			Phase: "implement",
			Text:  "--- FAIL: TestIdempotent\n",
			Data:  map[string]string{"error": "the gate said no"},
		},
		record.Event{
			At:   at(8),
			Kind: record.TaskFailed,
			Text: "the implement phase failed",
			Data: map[string]string{"error": "the gate said no"},
		},
