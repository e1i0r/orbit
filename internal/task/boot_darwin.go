...
// little-endian, which this comment used to describe as the machine's own
// byte order while the code below named one: every darwin Go builds for is
// little-endian (amd64 and arm64), so the two agree, and saying which one is
// being assumed is the point. Only the seconds are read: this is compared
