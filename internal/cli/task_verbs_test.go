...
func TestCancelTaskEarlyExits(t *testing.T) {
	...
	if code, _, errOut := run(t, "task", "cancel", "-repo", repoDir, "-nosuchflag"); code == 0 {
	...
	if code, _, errOut := run(t, "task", "cancel", "-repo", t.TempDir(), "ACME-1"); code == 0 {
	...
	if code, _, errOut := run(t, "task", "cancel", "-repo", repoDir, "ACME-404"); code == 0 {
...
func TestCancelNowKillsALiveProcess(t *testing.T) {
	...
	repoDir := filepath.Join(root, "payments")
	if code, _, errOut := run(t, "board", "new", "-repo", repoDir, "-id", "ACME-KILL", "kill me"); code != 0 {
		t.Fatalf("new exited %d: %s", code, errOut)
	}

	plantLiveMarker(t, "ACME-KILL")

	code, out, errOut := run(t, "task", "cancel", "-now", "-repo", repoDir, "ACME-KILL")
	if code != 0 {
...
func TestCancelGracefullyAsksALiveProcess(t *testing.T) {
	...
	repoDir := filepath.Join(root, "payments")
	if code, _, errOut := run(t, "board", "new", "-repo", repoDir, "-id", "ACME-ASK", "ask me"); code != 0 {
		t.Fatalf("new exited %d: %s", code, errOut)
	}

	plantLiveMarker(t, "ACME-ASK")

	code, out, errOut := run(t, "task", "cancel", "-repo", repoDir, "ACME-ASK")
	if code != 0 {
...
func TestControlTaskEarlyExits(t *testing.T) {
	...
	if code, _, errOut := run(t, "task", "pause", "-repo", t.TempDir(), "ACME-1"); code == 0 {
	...
	if code, _, errOut := run(t, "task", "resume", "-repo", repoDir, "ACME-404"); code == 0 {
...
func TestControlTaskFailsWhenTheTaskDirCannotBeWrittenTo(t *testing.T) {
	...
	code, _, errOut := run(t, "task", "pause", "-repo", dir, "ACME-1")
	if code == 0 {
...
func TestNoteTaskEarlyExits(t *testing.T) {
	...
	// 1. A flag parse failure.
	if code, _, errOut := run(t, "task", "note", "-repo", repoDir, "-nosuchflag"); code == 0 {
	...
	if code, _, errOut := run(t, "task", "note", "-repo", t.TempDir(), "ACME-1", "text"); code == 0 {
	...
	// 3. task.Load fails: a real repository, a task never written.
	if code, _, errOut := run(t, "task", "note", "-repo", repoDir, "ACME-404", "text"); code == 0 {
...
func TestNoteTaskFailsOverARecordItCannotReach(t *testing.T) {
	...
	code, _, errOut := run(t, "task", "note", "-repo", dir, "ACME-1", "a note nobody can write")
	if code == 0 {
