...
func TestDirectTaskEarlyExits(t *testing.T) {
	...
	// 1. Bad flag
	if code, _, errOut := run(t, "task", "direct", "-repo", repoDir, "-nosuchflag"); code == 0 {
	...
	// 2. Missing task id
	if code, _, errOut := run(t, "task", "direct", "-repo", repoDir); code == 0 {
	...
	// 3. Missing message
	if code, _, errOut := run(t, "task", "direct", "-repo", repoDir, "ACME-1"); code == 0 {
	...
	// 4. Unknown repo
	if code, _, errOut := run(t, "task", "direct", "-repo", t.TempDir(), "ACME-1", "msg"); code == 0 {
	...
	// 5. Unknown task
	if code, _, errOut := run(t, "task", "direct", "-repo", repoDir, "ACME-404", "msg"); code == 0 {
...
func TestDirectTaskRecordsDirective(t *testing.T) {
	...
	repoDir := filepath.Join(root, "payments")
	if code, _, errOut := run(t, "board", "new", "-repo", repoDir, "-id", "ACME-DIR", "direct me"); code != 0 {
		t.Fatalf("new exited %d: %s", code, errOut)
	}

	code, out, errOut := run(t, "task", "direct", "-repo", repoDir, "-by", "supervisor", "ACME-DIR", "switch to redis backend")
	if code != 0 {
...
func TestDirectTaskWithRestartFlag(t *testing.T) {
	...
	repoDir := filepath.Join(root, "payments")
	if code, _, errOut := run(t, "board", "new", "-repo", repoDir, "-id", "ACME-REOPEN", "reopen me"); code != 0 {
		t.Fatalf("new exited %d: %s", code, errOut)
	}

	code, out, errOut := run(t, "task", "direct", "-repo", repoDir, "-restart", "ACME-REOPEN", "retry with new plan")
	if code != 0 {
