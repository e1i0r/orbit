...
func joinable(t *testing.T) (pay string) {
	...
	if code, _, errOut := run(t, "board", "new", "-repo", pay, "-id", "ACME-1", "retry the webhook on 5xx"); code != 0 {
...
func TestJoinAnswersWithTheCheckoutToWorkIn(t *testing.T) {
	...
	code, out, errOut := run(t, "task", "join", "-repo", pay, "ledger")
	if code != 0 {
	...
	code, shown, errOut := run(t, "task", "show", "-repo", pay, "ACME-1")
	if code != 0 {
...
func TestJoinSaysWhatItIsMissing(t *testing.T) {
	...
	if code, _, errOut := run(t, "task", "join", "-repo", pay); code == 0 {
	...
	if code, _, errOut := run(t, "task", "join", "-repo", pay, "ledger"); code == 0 {
	...
	if code, _, errOut := run(t, "task", "join", "-repo", pay, "-task", "ACME-1", "warehouse"); code == 0 {
