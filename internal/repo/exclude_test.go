...
// TestARepoRuleTravelsWithTheRepo is the other half of what the exclude is
// for, and the half it used to get wrong.
//
// The window offers a checkout as somewhere to file a rule, with the words
// "travels with it, so whoever clones the project gets it". Excluding the
// whole of .orbit/ made that false: a rule filed against a repository never
// left the machine it was written on, so "this checkout" and "this machine"
// were the same answer with two names.
func TestARepoRuleTravelsWithTheRepo(t *testing.T) {
	r, wt := worktreeOf(t)
	...
	if !strings.Contains(in, ".orbit/knowledge/repo/never-log-a-card-number.md") {
		t.Errorf("the rule filed against this checkout did not travel with it; the commit holds:\n%s", in)
	}

	if strings.Contains(in, ".orbit/decisions/") {
		t.Errorf("a decision copy rode into the commit anyway:\n%s", in)
	}
}

// TestTheBroadLineIsNarrowedInPlace. A checkout an older Orbit already
// touched has ".orbit/" in its exclude, and a broader line already in the
// file goes on winning however narrow the new one is — so the fix would only
// ever have reached machines that had never run Orbit.
func TestTheBroadLineIsNarrowedInPlace(t *testing.T) {
	dir := t.TempDir()
	makeRepo(t, dir, "main", "")

	path := filepath.Join(dir, ".git", "info", "exclude")
	if err := os.MkdirAll(filepath.Dir(path), 0o755); err != nil {
		t.Fatalf("make info/: %v", err)
	}

	// What an older Orbit wrote, comment and all, with a line of somebody
	// else's above it.
	was := "*.swp\n" +
		"# Orbit's own copies of its decisions, which are not this\n" +
		"# project's files. The record they are copied from is the truth.\n" +
		".orbit/\n"

	if err := os.WriteFile(path, []byte(was), 0o600); err != nil {
		t.Fatalf("write the old exclude: %v", err)
	}
	...
	if strings.Contains(now, "\n.orbit/\n") {
		t.Errorf("the broad line is still there:\n%s", now)
	}

	if !strings.Contains(now, ".orbit/decisions/\n") {
		t.Errorf("the narrow line was not written:\n%s", now)
	}

	if !strings.Contains(now, "*.swp") {
		t.Errorf("somebody else's line was lost:\n%s", now)
	}

	// The comment stays above the line it explains, which is why the old
	// line is rewritten rather than removed and a new one appended.
	head := strings.Index(now, "# Orbit's own copies")
	if head < 0 || strings.Index(now, ".orbit/decisions/") < head {
		t.Errorf("the line is not under the comment that explains it:\n%s", now)
	}
}

// TestNarrowingHappensOnce, so that running Orbit twice does not leave two
// lines or undo the first.
func TestNarrowingHappensOnce(t *testing.T) {
	dir := t.TempDir()
	makeRepo(t, dir, "main", "")
	...
	now := read(t, filepath.Join(dir, ".git", "info", "exclude"))
	if n := strings.Count(now, ".orbit/decisions/"); n != 1 {
		t.Errorf("the line is in the file %d times:\n%s", n, now)
	}
}

// read is a file's whole text, for a test that asserts about it.
