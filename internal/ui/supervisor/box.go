...
func (s State) drawSupervisorInput(cw int, e Env) []string {
	p := e.Words
	...
	return append(s.inputLines(cw, e), "", theme.Paint(theme.Dim).Render(ways))
}
...
func (s State) inputLines(cw int, e Env) []string {
	prompt := theme.Paint(theme.Accent).Render("❯ ")

	if s.input == "" {
		placeholder := e.Words.T("supervisor.placeholder", "type a briefing, question or standing directive...")
