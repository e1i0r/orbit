...
func (s State) conversationRows(maxRows, cw int, e Env) []string {
	p := e.Words
	convs := conversationsOf(s.all)

	if len(convs) == 0 {
		return cells.Fill([]string{theme.Paint(theme.Dim).Render(cells.Fit(p.T("supervisor.no_conversations",
	...
	for i, c := range convs {
		...
		out = append(out, cells.Fit(mark+open+ink.Render(c.title), cw))
		out = append(out, cells.Fit("    "+theme.Paint(theme.Dim).Render(strings.Join([]string{
			s.lastSaid(c, e),
	...
	if len(out) <= maxRows {
		return cells.Fill(out, maxRows)
	}

	from := max(min(s.listSel*2-maxRows/2, len(out)-maxRows), 0)
...
func (s State) lastSaid(c conversation, e Env) string {
	p := e.Words
...
func (s State) conversationWays(e Env) []string {
	p := e.Words
