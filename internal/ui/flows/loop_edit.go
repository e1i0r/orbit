...
func (s *State) edited() *flow.Phase {
	ph := s.cur()
...
func (s *State) looping() bool {
	return s.cur().Loop != nil
}
...
func (s State) setLoopTurns(n int) State {
	ph := s.cur()
	if ph.Loop == nil {
		return s
	}
...
func (s State) setLoopChecks(text string) State {
	s.setChecks(text)
...
func (s *State) setChecks(text string) {
	ph := s.cur()
	if ph.Loop == nil {
		return
	}

	s.checksDraft, s.checksTyped = text, true
	s.checksFor = s.activePhase
...
func (s *State) loopChecksText() string {
	ph := s.cur()
	if ph.Loop == nil {
		return ""
	}

	if s.checksTyped && s.checksFor == s.activePhase {
		return s.checksDraft
	}
...
func (s *State) loopTurnsText() string {
	ph := s.cur()
	if ph.Loop == nil {
		return ""
	}
