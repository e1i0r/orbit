...
func TestDeleteSelectedFlowAndDeleteFlow(t *testing.T) {
	...
	m2, out := s.deleteSelectedFlow(e)
	if out.Cmd != nil || m2.confirmDelete {
		t.Fatalf("expected a no-op with nothing selected, got confirmDelete=%v cmd=%v", m2.confirmDelete, out.Cmd)
	}

	// 2. A built-in flow cannot be deleted. "careful" sorts first among the
	// three built-ins the fixture ships.
	s.sel = 0
	m2, m2Out := s.deleteSelectedFlow(e)
	wantBand(t, m2Out, "cannot be deleted")
	...
	wantBand(t, m2Out, "zzz-mine")

	// 4. deleteFlow itself, called directly for each origin.
	_, m3Out := s.deleteFlow("built-in-name", flow.OriginBuiltin, e)
	wantBand(t, m3Out, "cannot be deleted")

	m4, _ := s.deleteFlow("zzz-mine", flow.OriginUser, e)
	if !m4.confirmDelete {
...
func TestConfirmDeleteFlow(t *testing.T) {
	...
	s.sel = 0
	m2, m2Out := s.confirmDeleteFlow(e)
	wantBand(t, m2Out, "cannot be deleted")
	...
	// gone.
	if want := len(flow.BuiltinNames()) - 1; m2.sel != want {
		t.Errorf("sel after delete = %d, want %d (stepped back)", m2.sel, want)
	}
	...
	e.Flows = flowsTestDir(dir2)
	s.refresh(e.Flows)
	s.sel = 0 // aaa-mine sorts before every built-in
	m2, m2Out = s.confirmDeleteFlow(e)
	wantBand(t, m2Out, "aaa-mine")

	if m2.sel != 0 {
		t.Errorf("sel after delete = %d, want 0", m2.sel)
	}
}
...
func TestEditSelectedFlowOutOfRange(t *testing.T) {
	...
	m2, out := s.editSelectedFlow(e)
	if out.Cmd != nil || m2.creating {
...
func TestEditSelectedFlowOpensTheBuilder(t *testing.T) {
	...
	m2, m2Out := s.editSelectedFlow(e)
	if !m2.creating || !m2.isEditing {
		t.Fatalf("expected the builder open in edit mode")
	}

	if m2.flowName != "careful" {
		t.Errorf("flowName = %q, want careful", m2.flowName)
	}

	wantBand(t, m2Out, "careful")
}
...
func TestApplyFlowTemplate(t *testing.T) {
	...
	for _, c := range cases {
		m2, _ := s.applyFlowTemplate(c.tpl, e)
		if m2.flowName != c.wantName {
			t.Errorf("applyFlowTemplate(%q).flowName = %q, want %q", c.tpl, m2.flowName, c.wantName)
		}

		if len(m2.phases) != c.wantPhases {
			t.Errorf("applyFlowTemplate(%q) has %d phases, want %d", c.tpl, len(m2.phases), c.wantPhases)
		}
	}
	...
	m2, out := s.applyFlowTemplate("not-a-real-preset", e)
	if out.Cmd != nil {
		t.Errorf("expected nil cmd for an unknown template")
	}

	if len(m2.phases) != len(before.phases) || m2.flowName != before.flowName {
...
func TestDeletingAShadowSaysTheBuiltInIsBack(t *testing.T) {
	...
	for i, d := range flow.List(e.Flows) {
		if d.Name == "quick" {
			sel = i
		}
	}
	...
	_, m2Out := s.confirmDeleteFlow(e)
	wantBand(t, m2Out, "showing again")
}
