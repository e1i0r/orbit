...
func TestAPhaseBecomesALoopAndBackAgain(t *testing.T) {
	s, _ := editing(t)
	s.phases[0].Name = "fix"
	s.phases[0].Prompt = "read what failed"

	s = s.toggleLoop()

	loop := s.phases[0].Loop
	if loop == nil {
	...
	s = s.toggleLoop()
	if s.phases[0].Loop != nil || s.phases[0].Prompt != "read what failed" {
		t.Errorf("turning the loop off lost the phase: %+v", s.phases[0])
	}
}
...
func TestTheEnginesFieldsFollowThePhaseThatGoesRound(t *testing.T) {
	...
	s.edited().Prompt = "typed while looping"

	if got := s.phases[0].Loop.Phases[0].Prompt; got != "typed while looping" {
...
func TestTheChecksAreWrittenOnePerLine(t *testing.T) {
	s, _ := editing(t)
	s = s.toggleLoop()
	s = s.setLoopChecks("tests: go test ./...\ncoverage: make cover")

	until := s.phases[0].Loop.Until
	if len(until) != 2 {
	...
	if got := s.loopChecksText(); !strings.Contains(got, "coverage: make cover") {
...
func TestALoopWithNothingToStopItIsRefused(t *testing.T) {
	...
	_, _ = s.saveCustomFlow(e)
	if _, err := flow.Resolve(e.Flows, "mine"); err == nil {
...
func TestTheTurnsAreCappedAtSomethingReal(t *testing.T) {
	...
	if got := s.phases[0].Loop.Max; got < 1 {
...
func TestThePurposeHoldsAParagraph(t *testing.T) {
	...
	if s.description != "first line\nsecond line" {
		t.Errorf("the purpose holds %q", s.description)
	}

	// And the list, which has one row for it, shows it as one line.
	if got := flatten(s.description); got != "first line second line" {
