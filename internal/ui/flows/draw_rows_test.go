...
func TestFlowOriginString(t *testing.T) {
	...
	for _, c := range cases {
		got := OriginSaid(p, c.origin)
		if c.want == "" {
			if got != "" {
				t.Errorf("OriginSaid(%v) = %q, want empty", c.origin, got)
			}
		...
		if !strings.Contains(got, c.want) {
			t.Errorf("OriginSaid(%v) = %q, want it to mention %q", c.origin, got, c.want)
		}
	}
}
