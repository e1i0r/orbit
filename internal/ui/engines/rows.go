...
func (s State) collectEngineRows(e Env) []engineRow {
	p := e.Words

	var rows []engineRow

	engineList := e.catalogue()

	activeEngine := e.dialled(s.knobs.Engine)

	// 1. Model & Engine Section
	rows = append(rows, engineRow{kind: rowHeader, title: p.T("engines.section_model", "Engine & Model")})

	var currentEng roster.Engine
	...
	for _, eng := range engineList {
		...
		models := s.matchingModels(eng)
		if s.knobFilter() != "" && len(models) == 0 {
			continue
		}

		open := s.engineOpen(eng.Name) || s.knobFilter() != ""
	...
	if len(currentEng.Efforts) == 0 {
		...
		for _, eff := range currentEng.Efforts {
			...
			rows = append(rows, engineRow{
				...
				engine:   activeEngine,
				id:       eff.ID,
				selected: s.knobs.Effort == eff.ID,
			})
		}
	}
	...
	if !currentEng.CanThink {
		...
		rows = append(rows,
			engineRow{kind: rowThinking, title: "  " + p.T("engines.thinking_adaptive", "adaptive (default)"), id: "", selected: s.knobs.Thinking == "" || s.knobs.Thinking == "adaptive"},
			engineRow{kind: rowThinking, title: "  " + p.T("engines.thinking_on", "on"), id: "on", selected: s.knobs.Thinking == "on"},
			engineRow{kind: rowThinking, title: "  " + p.T("engines.thinking_off", "off"), id: "off", selected: s.knobs.Thinking == "off"},
		)
	}
