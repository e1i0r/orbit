...
// Region is one rectangle of the screen, in rows and cells. Y is its first
// row, counted from the top of the terminal; H is how many rows it owns; W
// is how many cells wide it is, which is the terminal's full width for all
// four regions the window has today.
//
// There is no X. Every region spans the terminal, and a region that did not
// would be a column layout — which is a different decision, and one this
// window has not made.
type Region struct {
	Y, H, W int
}
...
type Frame struct {
	Header Region // product, folder, and the standing settings
	Body   Region // the bands and their rows: the only region measured in tasks
	Band   Region // the activity band, which never empties
	Bar    Region // the keys that can be pressed right now
}
...
func Fit(w, h int) (Frame, error) {
	...
	for _, region := range []struct {
		...
		*region.into = Region{Y: y, H: region.height, W: w}
		y += region.height
	}
	return f, nil
}
