...
func TestALinkWithNoWordsShowsItsAddress(t *testing.T) {
	got := ansi.Strip(Inline("see []( https://example.com/x ) for more"))
...
func TestSomethingThatIsNotALinkIsLeftAlone(t *testing.T) {
	for _, said := range []string{
		...
		if got := ansi.Strip(Inline(said)); got != said {
			t.Errorf("Inline(%q) = %q, want it untouched", said, got)
		}
	}
}
