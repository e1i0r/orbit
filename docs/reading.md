...
Long lines scroll rather than wrap, since a generated file arrives as one line of
...
is not a rule and the pane says so, because a file left out on purpose is
the normal case. It is still the question a reviewer cannot ask a diff.
...
considered and did not do. Nobody verified it, since no command can check
"assumes UTC timestamps", and the discarded alternatives exist nowhere else. They die
...
answers, whether it broke, ran out or you stopped it, still says what it was asked.
...
`e` opens every row of any of them at once, and `v` shows what was written down
