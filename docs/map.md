...
| `internal/ui` | the cockpit. Draws; writes nothing. What it wants done goes out through a port. |
...
belongs to no checkout, so it stays in the state root and the price — nobody
else sees it — is paid knowingly.
