...
`quick`, `task`, `careful`, `coverage` and `tdd-fuzz-pr`, and you write your own:
...
is ordinary, since a phase turned down once that went another way did the work,
...
The work it did is still in the worktree, which is never lost, but the
...
|---|---|
| autopilot on | it changes to an engine with allowance and carries on, and you read about it afterwards |
...
always possible, and what was missing was the record saying it happened.
...
`errors.log`, and that text is what a new phrase should be added from. A phrase
