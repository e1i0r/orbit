...
4. **Integration** — when the thing being changed is the seam between parts: a task walking a whole flow, a command that opens the store and writes the record, a window driven through its ports. Only where the seam is the subject; a unit test wearing an integration costume is slower and proves less.
   ...
   `test/integration` is where the flows the landing shows are walked end to end, and `internal/cli/cockpit_flows_test.go` is where the six that are about the board rather than about a run are — the window `orbit top` builds, answering the keys the recordings press. Both are behind the same tag and both are run by `make integration`; a cockpit test has to be named `TestCockpit…`, because that prefix is what the target selects on.

   The run half: the real binary, a real git repository with a real Go module in it, real gates running real commands, and a stand-in engine on PATH under the name the flows ask for. Only the model is faked, because a model is neither free nor the same twice. They are behind a `//go:build integration` tag so `go test ./...` stays a second's work; `make integration` runs them and `make check` calls it, so a release cannot go out on flows nobody walked. A flow added to `internal/flow/flows` gets a file there.
5. **Mutation** — on the package a change touched, before the pull request: `make mutate PKG=./internal/ui/settings/...`. It changes the code under the tests and asks whether they notice. A mutant that survives is a statement no test disagrees with, which is a test watching without looking. Kill it or write down why it does not matter.
   ...
   **Read the timed-out count before the score.** gremlins computes efficacy over killed and lived mutants and leaves the timed-out ones out of it, so a run where almost everything timed out prints 100% and means nothing: `internal/db` once answered "Test efficacy: 100.00%" over 4 killed mutants and 188 timeouts. The target sets `--timeout-coefficient 8` for that reason, and `MUTATE_COEFFICIENT=16 make mutate PKG=…` raises it again for a package whose suite is slower still. A mutant that timed out was never tested.
